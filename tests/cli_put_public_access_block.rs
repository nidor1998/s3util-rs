//! Process-level CLI tests for the `put-public-access-block` subcommand.
...
fn help_shows_both_positionals() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["put-public-access-block", "--help"]));
    assert!(ok, "put-public-access-block --help must succeed");
    assert!(
        stdout.contains("TARGET") || stdout.contains("BUCKET"),
        "expected TARGET or BUCKET in help; got: {stdout}"
    );
    assert!(
        stdout.contains("PUBLIC_ACCESS_BLOCK_CONFIGURATION"),
        "expected PUBLIC_ACCESS_BLOCK_CONFIGURATION in help; got: {stdout}"
    );
...
fn missing_both_positionals_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("put-public-access-block"));
    assert!(!ok);
...
fn missing_config_positional_exits_2() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["put-public-access-block", "s3://example-bucket"]));
    assert!(!ok);
    assert_eq!(code, Some(2), "stderr: {stderr}");
}
...
fn nonexistent_config_file_exits_1() {
    let (ok, _stdout, stderr, code) = run(s3util().args([
        ...
        "s3://example-bucket",
        "/nonexistent/path/pab-xyz-does-not-exist.json",
    ]));
    assert!(!ok);
    assert_eq!(
        code,
...
fn auto_complete_shell_short_circuits_without_positionals() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["put-public-access-block", "--auto-complete-shell", "bash"]));
    assert!(ok);
