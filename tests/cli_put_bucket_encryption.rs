//! Process-level CLI tests for the `put-bucket-encryption` subcommand.
...
fn help_shows_both_positionals() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["put-bucket-encryption", "--help"]));
    assert!(ok, "put-bucket-encryption --help must succeed");
    assert!(
        stdout.contains("TARGET") || stdout.contains("BUCKET"),
        "expected TARGET or BUCKET in help; got: {stdout}"
    );
    assert!(
        stdout.contains("SERVER_SIDE_ENCRYPTION_CONFIGURATION"),
        "expected SERVER_SIDE_ENCRYPTION_CONFIGURATION in help; got: {stdout}"
    );
...
fn missing_both_positionals_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("put-bucket-encryption"));
    assert!(!ok);
...
fn missing_config_positional_exits_2() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["put-bucket-encryption", "s3://example-bucket"]));
    assert!(!ok);
    assert_eq!(code, Some(2), "stderr: {stderr}");
}
...
fn nonexistent_config_file_exits_1() {
    let (ok, _stdout, stderr, code) = run(s3util().args([
        ...
        "s3://example-bucket",
        "/nonexistent/path/encryption-xyz-does-not-exist.json",
    ]));
    assert!(!ok);
    assert_eq!(
        code,
...
fn auto_complete_shell_short_circuits_without_positionals() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["put-bucket-encryption", "--auto-complete-shell", "bash"]));
    assert!(ok);
