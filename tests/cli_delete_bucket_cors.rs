//! Process-level CLI tests for the `delete-bucket-cors` subcommand.
...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["delete-bucket-cors", "--help"]));
    assert!(ok);
...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("delete-bucket-cors"));
    assert!(!ok);
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["delete-bucket-cors", "--auto-complete-shell", "bash"]));
    assert!(ok);
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["delete-bucket-cors", "s3://example/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
