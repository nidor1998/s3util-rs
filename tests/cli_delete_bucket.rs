//! Process-level CLI tests for the `delete-bucket` subcommand.
...
fn help_succeeds_and_lists_option_groups() {
    ...
    assert!(ok, "delete-bucket --help must succeed");
...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("delete-bucket"));
    assert!(!ok);
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["delete-bucket", "--auto-complete-shell", "bash"]));
