//! Process-level CLI tests for the `put-object-tagging` subcommand.
...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["put-object-tagging", "--help"]));
    assert!(ok, "put-object-tagging --help must succeed");
...
fn missing_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("put-object-tagging"));
    assert!(!ok);
...
fn missing_tagging_flag_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().args(["put-object-tagging", "s3://bucket/key"]));
    assert!(!ok);
...
fn bucket_only_path_no_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["put-object-tagging", "s3://bucket", "--tagging", "k=v"]));
    assert!(!ok);
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["put-object-tagging", "--auto-complete-shell", "bash"]));
...
#[test]
fn help_mentions_tagging_and_source_version_id() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["put-object-tagging", "--help"]));
    assert!(ok);
    assert!(
