...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["get-object-tagging", "--help"]));
...
fn missing_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("get-object-tagging"));
    assert!(!ok);
...
fn bucket_only_path_no_key_exits_1() {
    let (ok, _stdout, stderr, code) = run(s3util().args(["get-object-tagging", "s3://bucket"]));
    assert!(!ok);
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["get-object-tagging", "--auto-complete-shell", "bash"]));
