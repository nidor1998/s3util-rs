...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("get-bucket-replication"));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["get-bucket-replication", "--auto-complete-shell", "bash"]));
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["get-bucket-replication", "s3://example/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
...
fn local_path_target_exits_1() {
    let (ok, _stdout, _stderr, code) =
        run(s3util().args(["get-bucket-replication", "/tmp/local-target"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
        "non-S3 path should exit 1 at bucket_name() validation"
    );
}
