//! Process-level CLI tests for the `put-bucket-logging` subcommand.
...
fn help_shows_both_positionals() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["put-bucket-logging", "--help"]));
    assert!(ok, "put-bucket-logging --help must succeed");
    assert!(
        stdout.contains("TARGET") || stdout.contains("BUCKET"),
        "expected TARGET or BUCKET in help; got: {stdout}"
    );
    assert!(
        stdout.contains("BUCKET_LOGGING_STATUS"),
        "expected BUCKET_LOGGING_STATUS in help; got: {stdout}"
    );
...
fn missing_both_positionals_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("put-bucket-logging"));
    assert!(!ok);
...
fn missing_config_positional_exits_2() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["put-bucket-logging", "s3://example-bucket"]));
    assert!(!ok);
    assert_eq!(code, Some(2), "stderr: {stderr}");
}
...
fn nonexistent_config_file_exits_1() {
    let (ok, _stdout, stderr, code) = run(s3util().args([
        "put-bucket-logging",
        "s3://example-bucket",
        "/nonexistent/path/logging-xyz-does-not-exist.json",
    ]));
    assert!(!ok);
    assert_eq!(
        code,
...
fn auto_complete_shell_short_circuits_without_positionals() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["put-bucket-logging", "--auto-complete-shell", "bash"]));
    assert!(ok);
