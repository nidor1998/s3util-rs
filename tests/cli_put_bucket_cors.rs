//! Process-level CLI tests for the `put-bucket-cors` subcommand.
...
fn help_shows_both_positionals() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["put-bucket-cors", "--help"]));
    assert!(ok, "put-bucket-cors --help must succeed");
    assert!(
        stdout.contains("TARGET") || stdout.contains("BUCKET"),
        "expected TARGET or BUCKET in help; got: {stdout}"
    );
    assert!(
        stdout.contains("CORS_CONFIGURATION"),
        "expected CORS_CONFIGURATION in help; got: {stdout}"
    );
...
fn missing_both_positionals_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("put-bucket-cors"));
    assert!(!ok);
...
fn missing_config_positional_exits_2() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["put-bucket-cors", "s3://example-bucket"]));
    assert!(!ok);
    assert_eq!(code, Some(2), "stderr: {stderr}");
}
...
fn nonexistent_config_file_exits_1() {
    let (ok, _stdout, stderr, code) = run(s3util().args([
        "put-bucket-cors",
        "s3://example-bucket",
        "/nonexistent/path/cors-xyz-does-not-exist.json",
    ]));
    assert!(!ok);
    assert_eq!(
        code,
...
fn auto_complete_shell_short_circuits_without_positionals() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["put-bucket-cors", "--auto-complete-shell", "bash"]));
    assert!(ok);
