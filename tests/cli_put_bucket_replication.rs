...
fn help_shows_both_positionals() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["put-bucket-replication", "--help"]));
    ...
    assert!(
        stdout.contains("REPLICATION_CONFIGURATION") || stdout.contains("REPLICATION"),
...
fn missing_both_positionals_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("put-bucket-replication"));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
...
fn missing_config_positional_exits_2() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["put-bucket-replication", "s3://example-bucket"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
        "missing config positional should exit 2; stderr: {stderr}"
    );
}
...
fn auto_complete_shell_short_circuits_without_positionals() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["put-bucket-replication", "--auto-complete-shell", "bash"]));
