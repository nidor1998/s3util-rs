...
mod tests {
    ...
    /// ID could trigger.
    #[tokio::test]
    async fn list_object_annotations_version_id_from_other_object_exits_4() {
        TestHelper::init_dummy_tracing_subscriber();
        ...
        let key = "version-nf-list-target.txt";
        helper
            .put_object_with_version(&bucket, key, b"current body".to_vec())
            .await;
        let other_version = helper
            .put_object_with_version(&bucket, "other-list-object.txt", b"other body".to_vec())
            .await;
        ...
        assert!(
            !out.status.success(),
            "list-object-annotations with a nonexistent version must not succeed"
        );
        assert_eq!(
            out.status.code(),
            Some(4),
