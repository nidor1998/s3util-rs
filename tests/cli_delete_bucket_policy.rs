//! Process-level CLI tests for the `delete-bucket-policy` subcommand.
...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["delete-bucket-policy", "--help"]));
    assert!(ok, "delete-bucket-policy --help must succeed");
...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("delete-bucket-policy"));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["delete-bucket-policy", "--auto-complete-shell", "bash"]));
