...
mod tests {
    ...
    async fn malformed_json_via_file_exits_1() {
        ...
        let tmp_dir = TestHelper::create_temp_dir();
        let config_file =
            TestHelper::create_test_file(&tmp_dir, "encryption.json", b"not valid json {");
        ...
        assert!(
            !out.status.success(),
            "put-bucket-encryption with invalid JSON file must fail"
        );
        assert_eq!(out.status.code(), Some(1));
    ...
    async fn malformed_json_via_stdin_exits_1() {
        ...
        assert!(
            !out.status.success(),
            "put-bucket-encryption with invalid JSON via stdin must fail"
        );
        assert_eq!(out.status.code(), Some(1));
        let stderr = String::from_utf8_lossy(&out.stderr);
        assert!(
