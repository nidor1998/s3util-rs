...
fn missing_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("rm"));
    assert!(!ok);
...
fn bucket_only_path_no_key_exits_1() {
    let (ok, _stdout, stderr, code) = run(s3util().args(["rm", "s3://bucket"]));
    assert!(!ok);
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["rm", "--auto-complete-shell", "bash"]));
...
fn target_access_key_without_secret_exits_non_zero() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["rm", "s3://bucket/key", "--target-access-key", "AKIA"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
