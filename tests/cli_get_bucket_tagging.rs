//! Process-level CLI tests for the `get-bucket-tagging` subcommand.
...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["get-bucket-tagging", "--help"]));
    assert!(ok, "get-bucket-tagging --help must succeed");
...
fn missing_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("get-bucket-tagging"));
    assert!(!ok);
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) = run(s3util().args(["get-bucket-tagging", "s3://bucket/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["get-bucket-tagging", "--auto-complete-shell", "bash"]));
