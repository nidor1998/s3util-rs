...
mod tests {
    ...
    async fn put_object_annotation_nonexistent_version_id_exits_4() {
        ...
        let key = "version-nf-put-target.txt";
        helper
            .put_object_with_version(&bucket, key, b"current body".to_vec())
            .await;
        let other_version = helper
            .put_object_with_version(&bucket, "other-put-object.txt", b"other body".to_vec())
            .await;

        let tmp_dir = TestHelper::create_temp_dir();
        let payload_file =
            TestHelper::create_test_file(&tmp_dir, "nv-payload.txt", b"no version payload");
        let payload_path = payload_file.to_str().unwrap();
        ...
        assert!(
            !out.status.success(),
            "put-object-annotation with a nonexistent version must not succeed"
        );
        assert_eq!(
            out.status.code(),
            Some(4),
