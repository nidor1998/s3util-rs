...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s3util().args(["get-bucket-encryption", "--help"]));
...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s3util().arg("get-bucket-encryption"));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
...
fn auto_complete_shell_short_circuits_without_target() {
    let (ok, stdout, _stderr, _code) =
        run(s3util().args(["get-bucket-encryption", "--auto-complete-shell", "bash"]));
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s3util().args(["get-bucket-encryption", "s3://example/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
