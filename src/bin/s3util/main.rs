...
async fn main() -> ExitCode {
    ...
    match cli_args.command {
        ...
        Commands::Presign(args) => {
            ...
            let exit_code = match cli::run_presign(args, client_config).await {
                Ok(status) => status.code(),
                Err(e) => {
                    tracing::error!(error = format!("{e:#}"));
                    cli::EXIT_CODE_ERROR
        ...
        Commands::PutBucketReplication(args) => {
            ...
            let exit_code = match cli::run_put_bucket_replication(args, client_config).await {
                Ok(()) => cli::EXIT_CODE_SUCCESS,
                Err(e) => {
                    tracing::error!(error = format!("{e:#}"));
                    cli::EXIT_CODE_ERROR
        ...
        Commands::GetBucketReplication(args) => {
            ...
            let exit_code = match cli::run_get_bucket_replication(args, client_config).await {
                Ok(status) => status.code(),
                Err(e) => {
                    tracing::error!(error = format!("{e:#}"));
                    cli::EXIT_CODE_ERROR
        ...
        Commands::DeleteBucketReplication(args) => {
            ...
            let exit_code = match cli::run_delete_bucket_replication(args, client_config).await {
                Ok(()) => cli::EXIT_CODE_SUCCESS,
                Err(e) => {
                    tracing::error!(error = format!("{e:#}"));
                    cli::EXIT_CODE_ERROR
        ...
        Commands::PutBucketAccelerateConfiguration(args) => {
            ...
            let exit_code =
                match cli::run_put_bucket_accelerate_configuration(args, client_config).await {
                    Ok(()) => cli::EXIT_CODE_SUCCESS,
                    Err(e) => {
                        tracing::error!(error = format!("{e:#}"));
                        cli::EXIT_CODE_ERROR
        ...
        Commands::GetBucketAccelerateConfiguration(args) => {
            ...
            let exit_code =
                match cli::run_get_bucket_accelerate_configuration(args, client_config).await {
                    Ok(status) => status.code(),
                    Err(e) => {
                        tracing::error!(error = format!("{e:#}"));
                        cli::EXIT_CODE_ERROR
        ...
        Commands::PutBucketRequestPayment(args) => {
            ...
            let exit_code = match cli::run_put_bucket_request_payment(args, client_config).await {
                Ok(()) => cli::EXIT_CODE_SUCCESS,
                Err(e) => {
                    tracing::error!(error = format!("{e:#}"));
                    cli::EXIT_CODE_ERROR
        ...
        Commands::GetBucketRequestPayment(args) => {
            ...
            let exit_code = match cli::run_get_bucket_request_payment(args, client_config).await {
                Ok(status) => status.code(),
                Err(e) => {
                    tracing::error!(error = format!("{e:#}"));
                    cli::EXIT_CODE_ERROR
