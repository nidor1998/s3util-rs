...
const TARGET_NOT_S3: &str = "delete-bucket-lifecycle-configuration target must be s3://<BUCKET>\n";
...
mod tests {
    ...
    fn rejects_local_path() {
        let res =
            TestCli::try_parse_from(["test", "delete-bucket-lifecycle-configuration", "/tmp/foo"]);
        if let Ok(cli) = res {
            let TestSub::DeleteBucketLifecycleConfiguration(a) = cli.cmd;
            assert!(a.bucket_name().is_err());
        }
