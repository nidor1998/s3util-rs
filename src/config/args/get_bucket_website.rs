...
const TARGET_NOT_S3: &str = "get-bucket-website target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "get-bucket-website target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct GetBucketWebsiteArgs {
    #[arg(
        env,
        help = "s3://<BUCKET_NAME>",
...
mod tests {
    ...
    fn parse(args: &[&str]) -> GetBucketWebsiteArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::GetBucketWebsite(a) = cli.cmd;
        a
    }

    #[test]
    fn accepts_bucket_only_path() {
        let a = parse(&["test", "get-bucket-website", "s3://my-bucket"]);
    ...
    fn rejects_path_with_key() {
        let a = parse(&["test", "get-bucket-website", "s3://my-bucket/key"]);
    ...
    fn bucket_name_rejects_non_s3_target() {
        let a = parse(&["test", "get-bucket-website", "/tmp/local"]);
