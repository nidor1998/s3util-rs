...
const TARGET_NOT_S3: &str = "get-bucket-notification-configuration target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "get-bucket-notification-configuration target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct GetBucketNotificationConfigurationArgs {
    #[arg(
        env,
        help = "s3://<BUCKET_NAME>",
...
impl GetBucketNotificationConfigurationArgs {
...
mod tests {
    ...
    fn parse(args: &[&str]) -> GetBucketNotificationConfigurationArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::GetBucketNotificationConfiguration(a) = cli.cmd;
        a
    }
