...
const TARGET_NOT_S3: &str = "get-bucket-logging target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "get-bucket-logging target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct GetBucketLoggingArgs {
    #[arg(
        env,
        help = "s3://<BUCKET_NAME>",
...
mod tests {
    ...
    fn parse(args: &[&str]) -> GetBucketLoggingArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::GetBucketLogging(a) = cli.cmd;
        a
    }

    #[test]
    fn accepts_bucket_only_path() {
        let a = parse(&["test", "get-bucket-logging", "s3://my-bucket"]);
    ...
    fn rejects_path_with_key() {
        let a = parse(&["test", "get-bucket-logging", "s3://my-bucket/key"]);
    ...
    fn bucket_name_rejects_non_s3_target() {
        let a = parse(&["test", "get-bucket-logging", "/tmp/local"]);
