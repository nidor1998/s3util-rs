...
const TARGET_NOT_S3: &str = "put-bucket-notification-configuration target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "put-bucket-notification-configuration target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct PutBucketNotificationConfigurationArgs {
    #[arg(
        env = "TARGET",
        help = "s3://<BUCKET_NAME>",
    ...
    /// Path to a file containing the notification-configuration JSON
    /// (AWS-CLI shape, with optional `TopicConfigurations`,
    /// `QueueConfigurations`, `LambdaFunctionConfigurations`, and
    /// `EventBridgeConfiguration`), or `-` to read from stdin. An empty
    /// object (`{}`) removes every notification on the bucket — there is
    /// no `delete-bucket-notification-configuration` API.
    #[arg(
        env = "NOTIFICATION_CONFIGURATION",
        required_unless_present = "auto_complete_shell"
    )]
...
impl PutBucketNotificationConfigurationArgs {
...
mod tests {
    ...
    fn parse(args: &[&str]) -> PutBucketNotificationConfigurationArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::PutBucketNotificationConfiguration(a) = cli.cmd;
        a
    }

    fn try_parse(args: &[&str]) -> Result<PutBucketNotificationConfigurationArgs, clap::Error> {
    ...
    fn accepts_bucket_and_file_path() {
        ...
        assert_eq!(
            a.notification_configuration.as_deref(),
            Some("/tmp/notification.json")
        );
    }
    ...
    fn accepts_bucket_and_stdin_dash() {
        ...
        assert_eq!(a.notification_configuration.as_deref(), Some("-"));
    }
    ...
    fn missing_both_positionals_errors() {
        let res = try_parse(&["test", "put-bucket-notification-configuration"]);
        assert!(res.is_err());
    }
    ...
    fn missing_positional_with_auto_complete_shell_is_ok() {
        ...
        assert!(a.notification_configuration.is_none());
