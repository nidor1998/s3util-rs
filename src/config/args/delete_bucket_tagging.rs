...
const TARGET_NOT_S3: &str = "delete-bucket-tagging target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "delete-bucket-tagging target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct DeleteBucketTaggingArgs {
    #[arg(
        env,
        help = "s3://<BUCKET_NAME>",
...
mod tests {
    ...
    fn parse(args: &[&str]) -> DeleteBucketTaggingArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::DeleteBucketTagging(a) = cli.cmd;
        a
    }

    #[test]
    fn accepts_bucket_only_path() {
        let a = parse(&["test", "delete-bucket-tagging", "s3://my-bucket"]);
    ...
    fn rejects_path_with_key() {
        let a = parse(&["test", "delete-bucket-tagging", "s3://my-bucket/key"]);
        assert!(a.bucket_name().is_err());
    }

    #[test]
    fn rejects_local_path() {
        let res = TestCli::try_parse_from(["test", "delete-bucket-tagging", "/tmp/foo"]);
        if let Ok(cli) = res {
            let TestSub::DeleteBucketTagging(a) = cli.cmd;
            assert!(a.bucket_name().is_err());
        }
    ...
    fn bucket_name_rejects_non_s3_target_via_parse() {
        let a = parse(&["test", "delete-bucket-tagging", "/tmp/local"]);
