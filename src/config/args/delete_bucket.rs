...
const TARGET_NOT_S3: &str = "delete-bucket target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "delete-bucket target must be s3://<BUCKET> with no key or prefix\n";
...
mod tests {
    ...
    fn parse(args: &[&str]) -> DeleteBucketArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::DeleteBucket(a) = cli.cmd;
        a
    }

    #[test]
    fn accepts_bucket_only_path() {
        let a = parse(&["test", "delete-bucket", "s3://my-bucket"]);
    ...
    fn rejects_path_with_key() {
        let a = parse(&["test", "delete-bucket", "s3://my-bucket/key"]);
        assert!(a.bucket_name().is_err());
    }

    #[test]
    fn rejects_local_path() {
        let res = TestCli::try_parse_from(["test", "delete-bucket", "/tmp/foo"]);
        if let Ok(cli) = res {
            let TestSub::DeleteBucket(a) = cli.cmd;
            assert!(a.bucket_name().is_err());
        }
    ...
    fn missing_positional_with_auto_complete_shell_is_ok() {
        let a = parse(&["test", "delete-bucket", "--auto-complete-shell", "bash"]);
