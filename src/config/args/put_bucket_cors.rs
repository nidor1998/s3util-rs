...
const TARGET_NOT_S3: &str = "put-bucket-cors target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "put-bucket-cors target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct PutBucketCorsArgs {
    #[arg(
        env = "TARGET",
        help = "s3://<BUCKET_NAME>",
    ...
    /// Path to a file containing the cors-configuration JSON (AWS-CLI shape,
    /// top-level `CORSRules` array), or `-` to read from stdin.
    #[arg(
        env = "CORS_CONFIGURATION",
        required_unless_present = "auto_complete_shell"
    )]
...
mod tests {
    ...
    fn parse(args: &[&str]) -> PutBucketCorsArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::PutBucketCors(a) = cli.cmd;
        a
    }

    fn try_parse(args: &[&str]) -> Result<PutBucketCorsArgs, clap::Error> {
    ...
    fn accepts_bucket_and_file_path() {
        ...
        assert_eq!(a.cors_configuration.as_deref(), Some("/tmp/cors.json"));
    }

    #[test]
    fn accepts_bucket_and_stdin_dash() {
        let a = parse(&["test", "put-bucket-cors", "s3://my-bucket", "-"]);
        assert_eq!(a.cors_configuration.as_deref(), Some("-"));
    }
    ...
    fn missing_config_positional_errors() {
        let res = try_parse(&["test", "put-bucket-cors", "s3://my-bucket"]);
        assert!(res.is_err());
    }

    #[test]
    fn missing_both_positionals_errors() {
        let res = try_parse(&["test", "put-bucket-cors"]);
        assert!(res.is_err());
    }

    #[test]
    fn missing_positional_with_auto_complete_shell_is_ok() {
        let a = parse(&["test", "put-bucket-cors", "--auto-complete-shell", "bash"]);
