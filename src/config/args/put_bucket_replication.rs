...
const TARGET_NOT_S3: &str = "put-bucket-replication target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "put-bucket-replication target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct PutBucketReplicationArgs {
    #[arg(
        env = "TARGET",
        help = "s3://<BUCKET_NAME>",
    ...
    /// Path to a file containing the replication-configuration JSON
    /// (AWS-CLI shape, top-level `Role` + `Rules`), or `-` to read from stdin.
    #[arg(
        env = "REPLICATION_CONFIGURATION",
        required_unless_present = "auto_complete_shell"
    )]
...
mod tests {
    ...
    fn parse(args: &[&str]) -> PutBucketReplicationArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::PutBucketReplication(a) = cli.cmd;
        a
    }

    fn try_parse(args: &[&str]) -> Result<PutBucketReplicationArgs, clap::Error> {
    ...
    fn accepts_bucket_and_file_path() {
        ...
        assert_eq!(
            a.replication_configuration.as_deref(),
            Some("/tmp/replication.json")
        );
    }

    #[test]
    fn accepts_bucket_and_stdin_dash() {
        let a = parse(&["test", "put-bucket-replication", "s3://my-bucket", "-"]);
        assert_eq!(a.replication_configuration.as_deref(), Some("-"));
    }
    ...
    fn missing_config_positional_errors() {
        let res = try_parse(&["test", "put-bucket-replication", "s3://my-bucket"]);
        assert!(res.is_err());
    }

    #[test]
    fn missing_both_positionals_errors() {
        let res = try_parse(&["test", "put-bucket-replication"]);
        assert!(res.is_err());
    }
    ...
    fn missing_positional_with_auto_complete_shell_is_ok() {
        ...
        assert!(a.replication_configuration.is_none());
