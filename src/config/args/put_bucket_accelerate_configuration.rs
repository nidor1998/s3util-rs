...
/// `put-bucket-versioning` pattern: clap enforces mutual exclusion, and the
/// "required" check is done post-parse via [`validate_state_flag`].
...
pub struct PutBucketAccelerateConfigurationArgs {
    ...
    /// Suspend Transfer Acceleration on the bucket
    #[arg(long)]
    pub suspended: bool,
...
impl PutBucketAccelerateConfigurationArgs {
    ...
    /// `--enabled` nor `--suspended` was supplied.
    ...
    /// Returns the accelerate status derived from the mutually-exclusive flags.
