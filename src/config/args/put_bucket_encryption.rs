...
const TARGET_NOT_S3: &str = "put-bucket-encryption target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "put-bucket-encryption target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct PutBucketEncryptionArgs {
    #[arg(
        env = "TARGET",
        help = "s3://<BUCKET_NAME>",
    ...
    /// Path to a file containing the server-side-encryption-configuration
    /// JSON (AWS-CLI shape, top-level `Rules` array), or `-` to read from
    /// stdin.
    #[arg(
        env = "SERVER_SIDE_ENCRYPTION_CONFIGURATION",
        required_unless_present = "auto_complete_shell"
    )]
...
mod tests {
    ...
    fn parse(args: &[&str]) -> PutBucketEncryptionArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::PutBucketEncryption(a) = cli.cmd;
        a
    }

    fn try_parse(args: &[&str]) -> Result<PutBucketEncryptionArgs, clap::Error> {
    ...
    fn accepts_bucket_and_file_path() {
        ...
        assert_eq!(
            a.server_side_encryption_configuration.as_deref(),
            Some("/tmp/encryption.json")
        );
    }

    #[test]
    fn accepts_bucket_and_stdin_dash() {
        let a = parse(&["test", "put-bucket-encryption", "s3://my-bucket", "-"]);
        assert_eq!(a.server_side_encryption_configuration.as_deref(), Some("-"));
    }
    ...
    fn missing_config_positional_errors() {
        let res = try_parse(&["test", "put-bucket-encryption", "s3://my-bucket"]);
        assert!(res.is_err());
    }

    #[test]
    fn missing_both_positionals_errors() {
        let res = try_parse(&["test", "put-bucket-encryption"]);
        assert!(res.is_err());
    }
