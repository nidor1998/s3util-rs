...
const TARGET_NOT_S3: &str = "get-bucket-cors target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "get-bucket-cors target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct GetBucketCorsArgs {
    #[arg(
        env,
        help = "s3://<BUCKET_NAME>",
...
mod tests {
    ...
    fn parse(args: &[&str]) -> GetBucketCorsArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::GetBucketCors(a) = cli.cmd;
        a
    }

    #[test]
    fn accepts_bucket_only_path() {
        let a = parse(&["test", "get-bucket-cors", "s3://my-bucket"]);
    ...
    fn rejects_path_with_key() {
        let a = parse(&["test", "get-bucket-cors", "s3://my-bucket/key"]);
        assert!(a.bucket_name().is_err());
    }

    #[test]
    fn missing_positional_with_auto_complete_shell_is_ok() {
        let a = parse(&["test", "get-bucket-cors", "--auto-complete-shell", "bash"]);
    ...
    fn bucket_name_rejects_non_s3_target() {
        // Local paths slip past the value_parser; bucket_name surfaces TARGET_NOT_S3.
        let a = parse(&["test", "get-bucket-cors", "/tmp/local"]);
