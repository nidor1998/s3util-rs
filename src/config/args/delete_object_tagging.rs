...
const TARGET_NOT_S3: &str = "delete-object-tagging target must be s3://<BUCKET>/<KEY>\n";
const TARGET_KEY_EMPTY: &str =
    "delete-object-tagging target must include a key (s3://<BUCKET>/<KEY>)\n";

#[derive(Parser, Clone, Debug)]
pub struct DeleteObjectTaggingArgs {
    #[arg(
        env,
...
mod tests {
    ...
    fn parse(args: &[&str]) -> DeleteObjectTaggingArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::DeleteObjectTagging(a) = cli.cmd;
        a
    }

    #[test]
    fn accepts_bucket_and_key() {
        let a = parse(&["test", "delete-object-tagging", "s3://my-bucket/my-key"]);
    ...
    fn rejects_bucket_only_path() {
        let a = parse(&["test", "delete-object-tagging", "s3://my-bucket"]);
