...
const TARGET_NOT_S3: &str = "get-bucket-accelerate-configuration target must be s3://<BUCKET>\n";
const TARGET_HAS_KEY_OR_PREFIX: &str =
    "get-bucket-accelerate-configuration target must be s3://<BUCKET> with no key or prefix\n";

#[derive(Parser, Clone, Debug)]
pub struct GetBucketAccelerateConfigurationArgs {
    #[arg(
        env,
        help = "s3://<BUCKET_NAME>",
...
impl GetBucketAccelerateConfigurationArgs {
...
mod tests {
    ...
    fn parse(args: &[&str]) -> GetBucketAccelerateConfigurationArgs {
        let cli = TestCli::try_parse_from(args).unwrap();
        let TestSub::GetBucketAccelerateConfiguration(a) = cli.cmd;
        a
    }
    ...
    fn bucket_name_rejects_non_s3_target() {
        let a = parse(&["test", "get-bucket-accelerate-configuration", "/tmp/local"]);
