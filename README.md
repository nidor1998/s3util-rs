...
The integration suite covers these paths without network access across 38 `tests/cli_*.rs` files (249 `#[test]` / `#[tokio::test]` annotations in total), including the cross-cutting `tests/cli_arg_validation.rs`, `tests/cli_config_validation_error.rs`, and `tests/cli_tracing_to_stderr.rs` plus a per-subcommand file for each `cp`, `mv`, `rm`, `head-*`, `create-bucket`, `delete-*`, `put-*`, and `get-*` runtime. They invoke the binary end-to-end and assert exit codes and stderr.
